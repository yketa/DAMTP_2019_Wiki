...
where
\begin{equation}
k^2 l^2 = \tau \psi \left(\frac{\tau \psi}{2} + 2\right)
\label{kPsi}
\end{equation}
...
\begin{equation}
\label{SCkl}
k^2 l^2 = 2 \tau \psi \Rightarrow k = \begin{cases} \frac{1}{l} \sqrt{2\tau\psi} &\text{ if } s > 0, \\ 0 &\text{ if } s = 0, \\ \mathrm{i} \, \frac{1}{l} \sqrt{- 2\tau\psi} &\text{ if } s < 0, \end{cases}
\end{equation}
give
\begin{eqnarray}
\frac{1}{s L v_0} = \frac{\coth\left(\sqrt{\frac{\psi L^2}{2 l v_0}}\right)}{\sqrt{\frac{\psi L^2}{2 l v_0}}} \Leftrightarrow \frac{1}{\Lambda} = \frac{\coth\left(\sqrt{\Phi}\right)}{\sqrt{\Phi}} &\qquad\qquad s > 0, \psi > 0\\
- \frac{1}{s L v_0} = \frac{\cot\left(\sqrt{\frac{|\psi| L^2}{2 l v_0}}\right)}{\sqrt{\frac{|\psi| L^2}{2 l v_0}}} \Leftrightarrow - \frac{1}{\Lambda} = \frac{\cot\left(\sqrt{|\Phi|}\right)}{\sqrt{|\Phi|}} &\qquad\qquad s < 0, \psi < 0
\end{eqnarray}
where we have set
\begin{eqnarray}
\label{Lambda}
\Lambda = s L v_0\\
\label{Phi}
\Phi = \frac{\psi L^2}{2 l v_0} = \frac{\Psi}{2 l v_0}
\end{eqnarray}
for convenience, and introduce
\begin{equation}
\Gamma = \frac{\Phi}{\Lambda} = \frac{\psi}{8 s v_0^2} \frac{4 L}{l} = \gamma \frac{4 L}{l}
\label{eqGamma}
\end{equation}
...
\caption{Rescaled sticking term $\Gamma = (4L/l) \, \gamma$ as a function of rescaled biasing parameter $\Lambda = s L v_0$.}
\label{Gamma}
\end{figure}
...
\begin{equation}
L \varepsilon(r) = \frac{1}{4} \Gamma \begin{cases} \frac{\cosh\left(\sqrt{\Phi}(1 - 2r/L)\right)}{\cosh\left(\sqrt{\Phi}\right)} &\text{ if } s > 0, \\ 1 &\text{ if } s = 0, \\ \frac{\cos\left(\sqrt{|\Phi|}(1 - 2r/L)\right)}{\cos\left(\sqrt{|\Phi|}\right)} &\text{ if } s < 0, \end{cases}
\end{equation}
...
\caption{Rescaled regular part $L \varepsilon(r)$ as a function of rescaled distance $r/L$ for different values of the rescaled biasing parameter $\Lambda = s L v_0$.}
\label{fig:epsilon}
\end{figure}
